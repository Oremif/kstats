...
class HypothesisTestBenchmark {
    ...
    @Benchmark fun kstatsOneSampleTTest(): TestResult = tTest(sample1, mu = 0.0)

    @Benchmark fun commonsOneSampleTTest(): Double = commonsTTest.tTest(0.0, sample1)

    // --- Two-sample t-test (Welch) ---

    @Benchmark fun kstatsTwoSampleTTest(): TestResult = tTest(sample1, sample2)

    @Benchmark fun commonsTwoSampleTTest(): Double = commonsTTest.tTest(sample1, sample2)

    // --- One-way ANOVA ---

    @Benchmark fun kstatsOneWayAnova(): AnovaResult = oneWayAnova(sample1, sample2, sample3)

    @Benchmark
    fun commonsOneWayAnova(): Double = commonsAnova.anovaPValue(listOf(sample1, sample2, sample3))

    // --- Chi-squared test ---

    @Benchmark fun kstatsChiSquared(): TestResult = chiSquaredTest(chiObserved, chiExpected)
    ...
    @Benchmark fun kstatsMannWhitney(): TestResult = mannWhitneyUTest(sample1, sample2)
    ...
    @Benchmark fun kstatsKolmogorovSmirnov(): TestResult = kolmogorovSmirnovTest(sample1, sample2)
