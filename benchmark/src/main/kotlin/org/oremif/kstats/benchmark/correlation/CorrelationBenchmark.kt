...
class CorrelationBenchmark {
    ...
    @Benchmark fun kstatsPearson(): CorrelationResult = pearsonCorrelation(x, y)

    @Benchmark fun commonsPearson(): Double = commonsPearson.correlation(x, y)

    // --- Spearman ---

    @Benchmark fun kstatsSpearman(): CorrelationResult = spearmanCorrelation(x, y)

    @Benchmark fun commonsSpearman(): Double = commonsSpearman.correlation(x, y)

    // --- Kendall Tau ---

    @Benchmark fun kstatsKendallTau(): CorrelationResult = kendallTau(x, y)

    @Benchmark fun commonsKendallTau(): Double = commonsKendall.correlation(x, y)
