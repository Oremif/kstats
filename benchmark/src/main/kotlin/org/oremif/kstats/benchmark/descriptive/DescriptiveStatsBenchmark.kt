...
class DescriptiveStatsBenchmark {
    ...
    @Param("100", "1000", "10000", "100000") var size: Int = 0
    ...
    @Benchmark fun kstatsMean(): Double = data.mean()

    @Benchmark fun commonsMean(): Double = StatUtils.mean(data)

    // --- Variance ---

    @Benchmark fun kstatsVariance(): Double = data.variance()

    @Benchmark fun commonsVariance(): Double = StatUtils.variance(data)

    // --- Standard Deviation ---

    @Benchmark fun kstatsStdDev(): Double = data.standardDeviation()

    @Benchmark fun commonsStdDev(): Double = commonsStdDev.evaluate(data)

    // --- Geometric Mean ---

    @Benchmark fun kstatsGeometricMean(): Double = positiveData.geometricMean()

    @Benchmark fun commonsGeometricMean(): Double = StatUtils.geometricMean(positiveData)

    // --- Median ---

    @Benchmark fun kstatsMedian(): Double = data.median()

    @Benchmark fun commonsMedian(): Double = commonsPercentile.evaluate(data, 50.0)

    // --- Skewness ---

    @Benchmark fun kstatsSkewness(): Double = data.skewness()

    @Benchmark fun commonsSkewness(): Double = commonsSkewness.evaluate(data)

    // --- Kurtosis ---

    @Benchmark fun kstatsKurtosis(): Double = data.kurtosis()

    @Benchmark fun commonsKurtosis(): Double = commonsKurtosis.evaluate(data)

    // --- Full Descriptive Statistics ---

    @Benchmark fun kstatsDescribe(): DescriptiveStatistics = data.describe()
