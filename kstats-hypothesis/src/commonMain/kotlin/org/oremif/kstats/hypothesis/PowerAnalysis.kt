 ...
 *
 * The test type determines how sample size `[n]` is interpreted and how the noncentrality parameter
 * is computed. For [TWO_SAMPLE], `[n]` is the per-group sample size and the noncentrality scales by
 * the square root of n/2. For [ONE_SAMPLE] and [PAIRED], `[n]` is the total sample size (or number
 * of pairs) and the noncentrality scales by the square root of n.
 */
public enum class TTestType {
     ...
     *
     * The parameter `[n]` represents the number of pairs. Uses the same noncentrality factor as
     * [ONE_SAMPLE].
     */
    PAIRED,
}
 ...
 *
 * Power is the probability of correctly rejecting the null hypothesis when the true effect size
 * equals [effectSize]. Higher power means the test is more likely to detect a real effect. A power
 * of 0.8 (80%) is the most common target in study design.
 *
 * Uses Cohen's d as the effect size metric. Conventional thresholds: 0.2 (small), 0.5 (medium), 0.8
 * (large). Negative effect sizes are treated as their absolute value. When [effectSize] is zero,
 * the returned power equals [alpha] (the Type I error rate).
 *
 * ### Example:
 * ```kotlin
 ...
 * @param n the sample size. For [TTestType.TWO_SAMPLE], this is the per-group size (assuming equal
 *   groups). For [TTestType.ONE_SAMPLE] and [TTestType.PAIRED], this is the total sample size or
 *   number of pairs. Must be at least 2.
 * @param alpha the significance level (Type I error rate). Defaults to `0.05` (5%). Must be in `(0,
 *   1)`.
 * @param alternative the direction of the alternative hypothesis. Defaults to
 *   [Alternative.TWO_SIDED]. One-sided tests have higher power for the same parameters.
...
public fun tTestPower(
    ...
    if (alpha <= 0.0 || alpha >= 1.0)
        throw InvalidParameterException("alpha must be in (0, 1), got $alpha")
 ...
 *
 * Given a target [power] and expected [effectSize] (Cohen's d), returns the smallest integer sample
 * size that meets or exceeds the requested power at significance level [alpha]. The result is
 * rounded up to the next whole number, with a minimum of 2.
 *
 * ### Example:
 * ```kotlin
 ...
 * @param power the target statistical power (probability of detecting the effect). Defaults to
 *   `0.8` (80%). Must be in `(0, 1)`.
 * @param alpha the significance level (Type I error rate). Defaults to `0.05` (5%). Must be in `(0,
 *   1)`.
 * @param alternative the direction of the alternative hypothesis. Defaults to
...
public fun tTestRequiredN(
    ...
    if (power <= 0.0 || power >= 1.0)
        throw InvalidParameterException("power must be in (0, 1), got $power")
    if (alpha <= 0.0 || alpha >= 1.0)
        throw InvalidParameterException("alpha must be in (0, 1), got $alpha")
    ...
    val nRaw =
        when (type) {
            ...
            TTestType.PAIRED -> (lambda / d) * (lambda / d)
 ...
 *
 * Returns the smallest Cohen's d that a t-test with [n] observations can detect at the specified
 * [power] and [alpha]. This is useful for assessing whether an existing dataset is large enough to
 ...
 * ```
 *
 * @param n the sample size. For [TTestType.TWO_SAMPLE], this is the per-group size. For
 *   [TTestType.ONE_SAMPLE] and [TTestType.PAIRED], this is the total size or number of pairs. Must
 *   be at least 2.
 ...
 *
 * Power is the probability of correctly rejecting the null hypothesis when the true effect size
 * equals [effectSize]. Uses Cohen's h as the effect size metric, which is based on the arcsine
 * transformation of proportions. Conventional thresholds: 0.2 (small), 0.5 (medium), 0.8 (large).
 * Negative effect sizes are treated as their absolute value.
 *
 * ### Example:
 * ```kotlin
 ...
 * @param n the sample size. For two-sample tests ([twoSample] = `true`), this is the per-group size
 *   assuming equal groups. For one-sample tests, this is the total sample size. Must be at least 2.
 * @param alpha the significance level (Type I error rate). Defaults to `0.05` (5%). Must be in `(0,
 *   1)`.
 * @param alternative the direction of the alternative hypothesis. Defaults to
 *   [Alternative.TWO_SIDED].
 * @param twoSample whether this is a two-sample test. Defaults to `true`. When `false`, uses the
 *   one-sample noncentrality factor (higher power for the same n).
 * @return the statistical power, a value in `[0, 1]`.
 ...
 *
 * Given a target [power] and expected [effectSize] (Cohen's h), returns the smallest integer sample
 * size that meets or exceeds the requested power at significance level [alpha]. The result is
 * rounded up to the next whole number, with a minimum of 2.
 *
 * ### Example:
 ...
 * @param alternative the direction of the alternative hypothesis. Defaults to
 *   [Alternative.TWO_SIDED].
 * @param twoSample whether this is a two-sample test. Defaults to `true`. When `true`, the returned
 *   value is the per-group sample size. When `false`, it is the total sample size.
...
/**
 * Computes the minimum detectable effect size (MDE) for a proportion z-test at the given sample
 * size.
 *
 * Returns the smallest Cohen's h that a proportion z-test with [n] observations can detect at the
 * specified [power] and [alpha]. This is useful for assessing whether an existing dataset or
 * planned study is large enough to detect practically meaningful differences between proportions.
 *
 * ### Example:
 * ```kotlin
 ...
 * ```
 *
 * @param n the sample size. For two-sample tests ([twoSample] = `true`), this is the per-group
 *   size. For one-sample tests, this is the total sample size. Must be at least 2.
 * @param power the target statistical power. Defaults to `0.8` (80%). Must be in `(0, 1)`.
 * @param alpha the significance level (Type I error rate). Defaults to `0.05` (5%). Must be in `(0,
 *   1)`.
 * @param alternative the direction of the alternative hypothesis. Defaults to
...
private fun tTestNoncentralityFactor(n: Int, type: TTestType): Double =
    when (type) {
        TTestType.ONE_SAMPLE,
        TTestType.PAIRED -> sqrt(n.toDouble())
...
private fun computePower(lambda: Double, alpha: Double, alternative: Alternative): Double {
    return when (alternative) {
        Alternative.TWO_SIDED -> {
            ...
            (standardNormal.cdf(lambda - zCrit) + standardNormal.cdf(-lambda - zCrit)).coerceIn(
                0.0,
                1.0,
            )
        }
...
private fun computeNoncentrality(power: Double, alpha: Double, alternative: Alternative): Double {
    ...
    val zCrit =
        when (alternative) {
            ...
            Alternative.LESS,
            Alternative.GREATER -> standardNormal.quantile(1.0 - alpha)
        }
    return zCrit + zBeta
}
