 ...
 *
 * Contains the point estimate (e.g. odds ratio or relative risk) together with a confidence
 * interval computed at the requested confidence level.
 *
 * ### Example:
 * ```kotlin
 ...
 * @property ci the confidence interval. Both bounds are [Double.NaN] when the interval cannot be
 *   computed (e.g. when a cell is zero or the confidence level is NaN).
 ...
 *
 * The odds ratio measures the strength of association between two binary variables. A value of 1.0
 * means no association, values above 1.0 indicate a positive association (exposure increases the
 * odds of the outcome), and values below 1.0 indicate a negative association. The Woolf confidence
 * interval is constructed on the log scale and exponentiated back, which ensures the interval is
 * always positive and handles skewed sampling distributions well.
 *
 * For a table `[[a, b], [c, d]]`, the odds ratio is `(a * d) / (b * c)`. When any cell is zero, the
 * point estimate follows IEEE 754 arithmetic (zero, infinity, or NaN) and the confidence interval
 * is `(NaN, NaN)` because the Woolf method requires all cells to be positive.
 *
 * ### Example:
 * ```kotlin
 ...
 * @param confidenceLevel the confidence level for the Woolf interval. Must be in `(0, 1)`. Defaults
 *   to `0.95` (95%).
 ...
 *
 * The relative risk (also called the risk ratio) compares the probability of the outcome in the
 * exposed group to the probability in the unexposed group. A value of 1.0 means equal risk, values
 * above 1.0 indicate higher risk in the first (exposed) group, and values below 1.0 indicate lower
 * risk. Unlike the odds ratio, the relative risk has a direct probabilistic interpretation and is
 * preferred when the outcome is common.
 *
 * For a table `[[a, b], [c, d]]`, the relative risk is `(a / (a + b)) / (c / (c + d))`. The
 * confidence interval is constructed on the log scale using the standard error of `log(RR)` and
 * exponentiated back. When the first cell in either row is zero, the confidence interval is `(NaN,
 * NaN)` because the log transform is undefined.
 *
 * ### Example:
 * ```kotlin
 * val table = arrayOf(intArrayOf(10, 20), intArrayOf(30, 40))
