 ...
 * making it appropriate for any sample size. The confidence interval method can be selected via
 * [ciMethod]: Clopper-Pearson (exact, conservative), Wilson (score-based, narrower), or
 * Agresti-Coull (adjusted Wald, simple approximation).
 *
 * ### Example:
 * ```kotlin
 ...
 *   [Alternative.TWO_SIDED]. [Alternative.LESS] tests if the true proportion is less than
 *   [probability], [Alternative.GREATER] tests if it is greater.
 * @param confidenceLevel the confidence level for the confidence interval. Must be in `(0, 1)`.
 *   Defaults to `0.95` (95%).
 * @param ciMethod the method used to compute the confidence interval for the proportion. Defaults
 *   to [CIMethod.CLOPPER_PEARSON] (exact interval). Use [CIMethod.WILSON] for narrower intervals
 *   recommended in A/B testing, or [CIMethod.AGRESTI_COULL] for a simple approximation.
 * @return a [TestResult] containing the observed proportion as the statistic, the exact p-value, a
 *   confidence interval computed using [ciMethod], and additional info with "successes", "trials",
    ...
    val ci =
        when (ciMethod) {
            CIMethod.CLOPPER_PEARSON -> {
                ...
                val cpLower =
                    if (k == 0) 0.0
                        ...
                        BetaDistribution(k.toDouble(), (n - k + 1).toDouble()).quantile(alpha / 2.0)
                val cpUpper =
                    if (k == n) 1.0
                    else
                        BetaDistribution((k + 1).toDouble(), (n - k).toDouble())
            ...
            CIMethod.WILSON -> {
                ...
                val halfWidth =
                    (z / denom) * sqrt(statistic * (1.0 - statistic) / n + z2 / (4.0 * n * n))
