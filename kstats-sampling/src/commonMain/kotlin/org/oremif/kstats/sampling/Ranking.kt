 ...
 *
 * When multiple elements share the same value, they occupy consecutive rank positions. This enum
 * determines which rank each tied element receives.
 */
public enum class TieMethod {
    /**
     * Assigns each tied element the average of the positions they occupy.
     *
     * For example, if two elements tie for positions 3 and 4, both receive rank 3.5. This is the
     * most common method and is used by default in [rank].
     */
    AVERAGE,
     ...
     *
     * If the sorted unique values are A < B < C, they receive ranks 1, 2, 3 regardless of how many
     * elements share each value. The maximum rank equals the number of distinct values.
     */
    DENSE,
     ...
     *
     * Tied elements receive different ranks depending on their original order. This is the only
     * method that guarantees all ranks are distinct.
     */
    ORDINAL,
}
 ...
 *
 * Ranking assigns a 1-based position to each value according to its magnitude. The smallest value
 * gets rank 1 (or a tie-adjusted rank), and ranks increase with value. The [tieMethod] parameter
 * controls how equal values share ranks.
 *
 * ### Example:
 * ```kotlin
 ...
 * ```
 *
 * @param tieMethod how to handle tied (equal) values. Defaults to [TieMethod.AVERAGE], which
 *   assigns each tied element the mean of the positions they occupy.
 * @return an array of ranks in the same order as the input, where each element's rank reflects its
...
/**
 * Computes the percentile rank of each element, indicating what percentage of values fall at or
 * below it.
 *
 * Percentile ranks are derived from average-method ranks and scaled to the range 0 to 100 using the
 * formula `(rank - 1) / (n - 1) * 100`. The smallest value receives 0 and the largest receives 100.
 * For a single-element array, the result is 0 (the sole element is the minimum by convention).
 *
 * This formula maps ranks linearly so that the minimum and maximum always land on 0 and 100
 * respectively, which differs from the SciPy convention `100 * (rank - 0.5) / n` that would assign
 * 50 to a single element.
 *
 * ### Example:
 * ```kotlin
 ...
 * ```
 *
 * @return an array of percentile ranks in the range [0, 100], preserving the original element
 *   order.
 * @throws InsufficientDataException if the array is empty.
 * @throws InvalidParameterException if the array contains NaN.
 */
