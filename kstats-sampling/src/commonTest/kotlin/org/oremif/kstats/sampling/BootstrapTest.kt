...
class BootstrapTest {
    ...
    fun testBootstrapCIMeanKnownDataset() {
        ...
        assertTrue(
            result.percentile.lower < result.observedStatistic,
            "percentile lower < observed",
        )
        assertTrue(
            result.percentile.upper > result.observedStatistic,
            "percentile upper > observed",
        )
    ...
    fun testBootstrapCINaNInData() {
        ...
        assertTrue(
            result.observedStatistic.isNaN(),
            "observed should be NaN when data contains NaN",
        )
    }
    ...
    fun testSymmetricDataBcaApproxPercentile() {
        ...
        val diff =
            abs(result.bca.lower - result.percentile.lower) +
    ...
    fun testBasicCIFormula() {
        // Verify basic (pivotal) CI formula: 2*observed - percentile(1-alpha/2), 2*observed -
        // percentile(alpha/2)
    ...
    fun testCustomStatisticStandardDeviation() {
        ...
        val data = doubleArrayOf(2.0, 4.0, 4.0, 4.0, 5.0, 5.0, 7.0, 9.0)
        val observedStd =
            sqrt(data.sumOf { (it - data.average()) * (it - data.average()) } / (data.size - 1))
