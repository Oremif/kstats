 ...
 *
 * Contains four standard SPC (Statistical Process Control) indices that measure how well a process
 * fits within its specification limits. Values above 1.0 indicate a capable process; values above
 * 1.33 are generally considered good.
 *
 * Cp and Cpk use the sample standard deviation (divides by n-1) as the spread estimate, while Pp
 * and Ppk use the population standard deviation (divides by n).
 *
 * @property cp potential capability — compares the tolerance width to 6 times the sample standard
 *   deviation. Ignores process centering.
 ...
 * within specification limits. The lower specification limit [lsl] and upper specification limit
 * [usl] define the acceptable range. The indices compare this tolerance width to the observed
 * spread of the data:
 *
...
public fun DoubleArray.processCapability(lsl: Double, usl: Double): ProcessCapabilityResult {
    if (size < 2)
        throw InsufficientDataException(
            "Process capability requires at least 2 elements, got $size"
        )
    if (lsl >= usl)
        throw InvalidParameterException("lsl must be less than usl, got lsl=$lsl, usl=$usl")
    ...
    return welford { mean, m2 ->
        ...
        val ppk =
            min((usl - mean) / (3.0 * populationSigma), (mean - lsl) / (3.0 * populationSigma))
 ...
 *
 * These indices measure whether a process produces output that fits within specification limits
 * defined by [lsl] (lower) and [usl] (upper). Values above 1.0 indicate a capable process; values
 * above 1.33 are generally considered good.
 *
 * NaN values in the data propagate through the computation (IEEE 754 semantics).
 *
 * ### Example:
