...
/**
 * Throws [InsufficientDataException] if this collection is empty. Returns the collection for
 * chaining.
 */
...
/**
 * Throws [InsufficientDataException] if this collection has fewer than [minSize] elements. Returns
 * the collection for chaining.
 */
internal fun <T> Collection<T>.requireMinSize(
    minSize: Int,
    name: String = "collection",
): Collection<T> {
    if (size < minSize)
        throw InsufficientDataException("$name must have at least $minSize elements, got $size")
    return this
}

/**
 * Throws [InsufficientDataException] if this array has fewer than [minSize] elements. Returns the
 * array for chaining.
 */
 ...
 *
 * NaN passes validation intentionally (`NaN <= 0.0` is `false` per IEEE 754). This allows NaN to
 * propagate through subsequent computation rather than being rejected at the validation boundary.
 */
...
internal fun requireInRange(value: Double, min: Double, max: Double, name: String = "value") {
    if (value !in min..max)
        throw InvalidParameterException("$name must be in [$min, $max], got $value")
}
