...
class DocsSamples {
    ...
    fun qcControlLimits() {
        ...
        val outOfControl = sensorReadings.filter {
            it > upperControlLimit || it < lowerControlLimit
        }
        // SampleEnd
    }
    ...
    fun edaSummaryStats() {
        ...
        rtSummary.mean
        rtSummary.standardDeviation
        rtSummary.min
        rtSummary.max
        errSummary.mean
        errSummary.standardDeviation
        errSummary.min
        errSummary.max
        memSummary.mean
        memSummary.standardDeviation
        memSummary.min
        memSummary.max
        tpSummary.mean
        tpSummary.standardDeviation
        tpSummary.min
        tpSummary.max
        // SampleEnd
    }
    ...
    fun coreWesternElectricRules() {
        ...
        // SampleEnd
        assertTrue(
            violations.rule1.isNotEmpty() ||
                violations.rule2.isNotEmpty() ||
                violations.rule3.isNotEmpty() ||
                violations.rule4.isNotEmpty()
        )
    }
