...
/**
 * Represents Student's t-distribution, a continuous probability distribution defined on the entire
 * real line.
 *
 * Student's t-distribution arises when estimating the mean of a normally distributed population
 * whose standard deviation is unknown and must be estimated from the data. It is the foundation of
 * t-tests and confidence intervals for means. The distribution resembles the standard normal
 * distribution but has heavier tails, which accounts for the additional uncertainty from estimating
 * the standard deviation. As [degreesOfFreedom] increases, the t-distribution converges to the
 * standard normal distribution.
 *
 * The CDF is computed via the regularized incomplete beta function. Quantiles are found using
 * Newton's method seeded with the corresponding standard normal quantile. Random samples are
 * generated as the ratio of a standard normal draw to the square root of an independent chi-squared
 * draw divided by its degrees of freedom.
 *
 * ### Example:
 * ```kotlin
 ...
 */
public class StudentTDistribution(public val degreesOfFreedom: Double) : ContinuousDistribution {

    init {
        if (!degreesOfFreedom.isFinite() || degreesOfFreedom <= 0.0)
            throw InvalidParameterException(
                "Degrees of freedom must be finite and positive, got $degreesOfFreedom"
            )
    }
     ...
     *
     * The density is symmetric about zero and approaches the standard normal density as degrees of
     * freedom increases.
     *
     ...
     *
     * Returns the probability that a t-distributed random variable is less than or equal to [x],
     ...
     *
     * Uses the symmetry of the t-distribution through the regularized incomplete beta function for
     * improved numerical accuracy in each tail.
     *
     ...
     *
     * Uses Newton's method seeded with the corresponding standard normal quantile. The distribution
     * is symmetric, so `quantile(0.5)` returns exactly 0.
     *
    ...
    /**
     * The mean of this distribution, which is 0 when degrees of freedom exceeds 1, or NaN
     * otherwise.
     */
    override val mean: Double
        get() = if (df > 1) 0.0 else Double.NaN

    /**
     * The variance of this distribution. Finite when df > 2, infinite when 1 < df <= 2, or NaN when
     * df <= 1.
     */
    ...
    /**
     * The skewness of this distribution, which is 0 when degrees of freedom exceeds 3, or NaN
     * otherwise.
     */
    override val skewness: Double
        get() = if (df > 3) 0.0 else Double.NaN

    /**
     * The excess kurtosis of this distribution. Finite when df > 4, infinite when 2 < df <= 4, or
     * NaN when df <= 2.
     */
     ...
     *
     * Generates an independent standard normal sample and a chi-squared sample, then returns their
     * ratio scaled by the degrees of freedom. This is the definition-based sampling method.
     *
