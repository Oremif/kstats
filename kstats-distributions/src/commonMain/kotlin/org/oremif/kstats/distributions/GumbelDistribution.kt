...
/**
 * The Gumbel distribution (extreme value type I, right-skewed), defined by its location [mu] and
 * scale [beta] parameters.
 *
 * The Gumbel distribution models the distribution of the maximum of a number of samples of various
 * distributions. It is widely used in hydrology (flood analysis), meteorology (extreme weather
 * events), and reliability engineering (failure analysis). Unlike symmetric distributions, the
 * Gumbel has a longer right tail, reflecting the tendency for extreme maxima to be farther from the
 * center than extreme minima.
 *
 ...
 * @property beta the scale parameter (spread) of the distribution. Must be positive. Defaults to
 *   `1.0`.
 */
public class GumbelDistribution(
     ...
     *
     * The density is computed using the standardized variable z = (x - mu) / beta as exp(-(z +
     * exp(-z))) / beta. The left tail decays doubly-exponentially (much faster than the right
     * tail), so density values near the left extreme may underflow to zero.
     *
     ...
     *
     * The CDF has the double-exponential form exp(-exp(-z)), where z = (x - mu) / beta. This gives
     * the probability that a random value from this distribution is less than or equal to [x].
     *
     ...
     *
     * Computed as -expm1(-exp(-z)) to avoid catastrophic cancellation when the CDF is close to 1
     * (i.e., in the right tail).
     *
     ...
     *
     * The quantile is computed as mu - beta * ln(-ln(p)). Returns negative infinity for p = 0 and
     * positive infinity for p = 1.
     *
    ...
    override val mean: Double
        get() = mu + beta * EULER_MASCHERONI

    /** The variance of this distribution, equal to (pi * beta)^2 / 6. */
    override val variance: Double
        get() = PI * PI * beta * beta / 6.0

    /** The standard deviation of this distribution, equal to pi * beta / sqrt(6). */
    override val standardDeviation: Double
        get() = PI * beta / sqrt(6.0)

    /**
     * The skewness of this distribution, equal to 12 * sqrt(6) * zeta(3) / pi^3, approximately
     * 1.1395.
     */
    override val skewness: Double
        get() = 1.1395470994717452
