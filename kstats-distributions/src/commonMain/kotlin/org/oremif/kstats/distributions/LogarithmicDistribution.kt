...
/**
 * Represents the logarithmic (log-series) distribution, a discrete power-series distribution on the
 * positive integers {1, 2, 3, ...}.
 *
 * The probability of observing k is proportional to p^k / k. This distribution arises as the
 * conditional distribution of the number of occurrences given at least one occurrence, and commonly
 * models species abundance in ecology (Fisher's logarithmic series), word frequency distributions,
 * and cascade failures in networks.
 *
 * The PMF is `f(k) = -p^k / (k * ln(1 - p))` for k = 1, 2, 3, ...
 *
 * The normalization constant is `-1 / ln(1 - p)`, derived from the Maclaurin series `-ln(1 - p) =
 * Σ_{k=1}^{∞} p^k / k`.
 *
 * ### Example:
 * ```kotlin
 ...
 */
public class LogarithmicDistribution(public val probability: Double) : DiscreteDistribution {
     ...
     *
     * Computed by accumulating PMF values from k=1 using the recurrence `PMF(k) = PMF(k-1) * p *
     * (k-1) / k`.
     *
     ...
     *
     * Uses a linear search accumulating PMF values from k=1. For distributions with very high
     * probability parameter (p close to 1), the search is capped at 1,000,000 iterations and
     * returns the current position, which may underestimate the true quantile.
     *
     * @param p the cumulative probability, must be in `[0, 1]`.
     * @return the smallest integer k in the support at which `cdf(k) >= p`. Returns [Int.MAX_VALUE]
     *   when `p = 1.0` (infinite support).
     */
    ...
    override val mean: Double
        get() = a * probability / (1.0 - probability)
     ...
     *
     * Computed from closed-form raw moments E[X^r] = a * p * S_r / (1-p)^r where S_1=1, S_2=1,
     * S_3=(1+p), S_4=(1+4p+p²).
     */
    ...
    override val kurtosis: Double
        get() {
            ...
            val mu3p = a * probability * (1.0 + probability) / (q * q * q)
            val mu4p =
                a * probability * (1.0 + 4.0 * probability + probability * probability) /
                    (q * q * q * q)
            val v = mu2p - mu1 * mu1
            val m4 = mu4p - 4.0 * mu1 * mu3p + 6.0 * mu1 * mu1 * mu2p - 3.0 * mu1 * mu1 * mu1 * mu1
            return m4 / (v * v) - 3.0
        }

    /**
     * The Shannon entropy of this distribution in nats, computed by summing over the support until
     * cumulative probability is within 1e-15 of 1.0.
     */
    ...
    /**
     * Draws a single random value from this logarithmic distribution using the Kemp (1981)
     * algorithm, which runs in O(1) expected time per draw.
     *
