...
/**
 * Represents the triangular distribution, a continuous probability distribution defined by a lower
 * limit [a], an upper limit [b], and a mode (peak) [c].
 *
 * The triangular distribution has a piecewise linear density that rises from zero at [a] to a peak
 * at [c], then falls back to zero at [b]. It is commonly used in simulation and risk analysis when
 * only the minimum, maximum, and most likely value of a quantity are known, making it a practical
 * alternative to the beta distribution when limited information is available.
 *
 * The PDF consists of two linear segments joined at the mode [c], the CDF is piecewise quadratic,
 * and the quantile function uses the inverse of those quadratic segments. Sampling is performed via
 ...
 * ```
 *
 * @property a the lower limit (minimum) of the distribution's support. Must be strictly less than
 *   [b].
 * @property b the upper limit (maximum) of the distribution's support. Must be strictly greater
 *   than [a].
...
public class TriangularDistribution(
     ...
     *
     * Returns `2(x - a) / ((b - a)(c - a))` for `a <= x < c`, `2 / (b - a)` at `x = c`, `2(b - x) /
     * ((b - a)(b - c))` for `c < x <= b`, and `0.0` outside the support.
     *
     ...
     *
     * Uses the logarithmic form of the piecewise density to avoid computing the density and then
     * taking its log, which improves numerical stability for values near the boundaries.
     *
     ...
     *
     * Returns `(x - a)^2 / ((b - a)(c - a))` for `a < x <= c` and `1 - (b - x)^2 / ((b - a)(b -
     * c))` for `c < x < b`.
     *
     ...
     *
     * Uses the inverse of the piecewise quadratic CDF. For `p <= (c - a) / (b - a)`, the result is
     * computed from the left segment; otherwise, from the right segment.
     *
    ...
    override val variance: Double
        get() = (a * a + b * b + c * c - a * b - a * c - b * c) / 18.0

    /**
     * The skewness of this distribution, computed from [a], [b], and [c]. Zero when the
     * distribution is symmetric (`c = (a + b) / 2`).
     */
    ...
    /**
     * The excess kurtosis of this distribution, always `-0.6` for any triangular distribution
