...
/**
 * Represents the chi-squared distribution, a continuous probability distribution defined on the
 * interval [0, +infinity).
 *
 * The chi-squared distribution describes the distribution of a sum of squared standard normal
 * random variables. It is one of the most widely used distributions in statistical inference,
 * appearing in chi-squared goodness-of-fit tests, tests of independence in contingency tables, and
 * confidence intervals for population variance. The single parameter [degreesOfFreedom] determines
 * the shape: higher values shift the distribution to the right and make it more symmetric.
 *
 * Internally, this distribution is implemented as a special case of the Gamma distribution with
 * shape equal to half the degrees of freedom and rate equal to 0.5. The CDF, quantile, and sampling
 * methods all delegate to this underlying Gamma parameterization.
 *
 * ### Example:
 * ```kotlin
 ...
 */
public class ChiSquaredDistribution(public val degreesOfFreedom: Double) : ContinuousDistribution {
     ...
     *
     * Returns zero for negative values. At x = 0, the density is 0.5 when degrees of freedom equals
     * 2, infinite when less than 2, and zero when greater than 2.
     *
     ...
     *
     * Returns the probability that a chi-squared-distributed random variable is less than or equal
     * to [x], evaluated via the regularized lower incomplete gamma function.
     *
     ...
     *
     * Uses Newton's method seeded with a Wilson-Hilferty normal approximation for degrees of
     * freedom greater than 2, or the midpoint of the distribution for smaller values.
     *
     ...
     *
     * Delegates to the Gamma distribution with shape = df/2 and rate = 0.5, which is mathematically
     * equivalent.
     *
