 ...
 *
 * Every probability distribution — whether continuous or discrete — extends this interface, which
 * provides shared statistical properties such as the mean, variance, standard deviation, skewness,
 * kurtosis, and entropy. It also declares the cumulative distribution function (CDF), the survival
 * function (SF), and the quantile (inverse CDF) function.
 *
 * Concrete distributions do not implement this interface directly. Instead, they implement either
 * [ContinuousDistribution] or [DiscreteDistribution], both of which extend [Distribution] with
 * type-specific methods like `pdf`/`pmf` and `sample`.
 *
 * ### Example:
 * ```kotlin
...
public sealed interface Distribution {
    ...
    /**
     * The variance (second central moment) of this distribution, measuring how spread out values
     * are around the [mean].
     */
    ...
    /**
     * The skewness (third standardized moment) of this distribution, measuring asymmetry around the
     * [mean]. Zero for symmetric distributions, positive for right-skewed, negative for
     * left-skewed.
     */
    public val skewness: Double

    /**
     * The excess kurtosis (fourth standardized moment minus 3) of this distribution, measuring the
     * heaviness of the tails relative to a normal distribution. Zero for normal, positive for
     * heavier tails.
     */
    public val kurtosis: Double

    /**
     * The Shannon entropy of this distribution in nats (natural logarithm units), measuring the
     * uncertainty or spread of the distribution.
     */
     ...
     *
     * The CDF returns the probability that a random variable drawn from this distribution takes a
     ...
     * distribution takes a value strictly greater than [x]. It is the complement of the CDF, that
     * is, `sf(x) = 1 - cdf(x)`. Some distributions override this default to provide a numerically
     * more accurate result in the upper tail.
     *
     ...
     *
     * Returns the smallest value x such that `cdf(x) >= p`. This is the inverse of the cumulative
     * distribution function: given a probability, it returns the corresponding threshold value.
     *
