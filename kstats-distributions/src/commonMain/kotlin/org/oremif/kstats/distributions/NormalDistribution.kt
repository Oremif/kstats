 ...
 * controlled by the standard deviation [sigma]. It arises naturally as the limiting distribution of
 * sums of independent random variables (central limit theorem) and is the foundation for many
 * statistical methods including confidence intervals, hypothesis tests, and regression.
 *
 * About 68% of values fall within one standard deviation of the mean, about 95% within two, and
 * about 99.7% within three. The distribution is fully supported on the entire real line, meaning
 * any real number is a possible outcome, though values far from the mean are exceedingly rare.
 *
...
public class NormalDistribution(
     ...
     *
     * This override uses `erfc` directly rather than `1 - cdf(x)`, which provides better numerical
     * accuracy in the upper tail where the CDF is close to 1.
     *
    ...
    /**
     * Computes the quantile (inverse CDF) for the given probability [p] using the inverse error
     * function.
     *
     * @param p the cumulative probability, must be in `[0, 1]`.
     * @return the value x at which `cdf(x) = p`. Returns [Double.NEGATIVE_INFINITY] for `p = 0` and
    ...
    /**
     * The excess kurtosis of this distribution, always zero by definition (the normal distribution
     * is the reference).
     */
    override val kurtosis: Double
        get() = 0.0 // excess kurtosis
     ...
     * independent standard normal random values. One of those values is then scaled by [sigma] and
     * shifted by [mu].
     *
