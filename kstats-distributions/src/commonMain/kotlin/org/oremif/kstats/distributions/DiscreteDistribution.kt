 ...
 *
 * A discrete distribution assigns probabilities to individual integer outcomes via a probability
 * mass function (PMF). Implementations provide methods to evaluate the mass, log-mass, cumulative
 * probability, quantiles, and random sampling.
 *
 * Extends [Distribution], which provides shared statistical properties such as [mean], [variance],
 * [standardDeviation], and the [sf] survival function. The [cdf] and [sf] methods are available in
 * both `Int` and `Double` overloads — the `Double` variants delegate to the `Int` versions via
 * truncation.
 *
 * ### Example:
 * ```kotlin
...
public interface DiscreteDistribution : Distribution {
     ...
     *
     * The probability mass is the exact probability that the random variable equals [k]. Returns
     * zero for values outside the distribution's support.
     *
     ...
     *
     * The log-mass is useful when working with very small probabilities that would underflow to
     * zero in regular floating-point arithmetic. Every implementation provides a direct formula
     * rather than computing `ln(pmf(k))`, which avoids unnecessary precision loss.
     *
     ...
     * than or equal to [k]. This is the sum of [pmf] values from the smallest supported value up to
     * [k].
     *
     ...
     *
     * The survival function is the probability that the random variable is strictly greater than
     * [k], equal to `1 - cdf(k)`.
     *
     ...
     *
     * Floors [x] to an integer via [floor] and delegates to [sf]. Returns [Double.NaN] if [x] is
     * NaN.
     *
     ...
     *
     * Floors [x] to an integer via [floor] and delegates to [cdf]. This override satisfies the
     * [Distribution] interface contract, which declares `cdf(x: Double)`.
     *
     ...
     *
     * The quantile function returns the smallest integer k such that `cdf(k) >= p`. This is the
     * type-safe integer variant — use [quantile] for [Double] compatibility with the [Distribution]
     * interface.
     *
     ...
     *
     * Delegates to [quantileInt] and converts the result to [Double]. This override satisfies the
     * [Distribution] interface contract. Prefer [quantileInt] when an integer result is needed.
     *
     * @param p the cumulative probability, must be in `[0, 1]`.
     ...
     *
     * Returns [Double.NaN] by default because not all discrete distributions have closed-form
     * skewness expressions. Implementations override this with the exact formula when available.
     *
     * @return the skewness, or [Double.NaN] if not available for this distribution.
     */
     ...
     *
     * Returns [Double.NaN] by default because not all discrete distributions have closed-form
     * kurtosis expressions. Implementations override this with the exact formula when available.
     *
     * @return the excess kurtosis, or [Double.NaN] if not available for this distribution.
     */
