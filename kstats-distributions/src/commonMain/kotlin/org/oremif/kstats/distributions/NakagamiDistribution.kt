...
/**
 * Represents the Nakagami-m distribution, a continuous probability distribution defined on the
 * interval [0, +infinity).
 *
 * The Nakagami distribution models the magnitude of signals subject to multipath fading and is
 * widely used in wireless communications. If X ~ Nakagami(mu, omega), then X squared follows a
 * Gamma distribution with shape [mu] and scale omega/mu. When [mu] = 1 the distribution reduces to
 * Rayleigh, and when [mu] = 0.5 it reduces to a half-normal.
 *
 * The CDF is computed via the regularized incomplete gamma function using the Gamma relationship.
 * Quantiles are found using Newton's method with a mean-based initial guess. Random samples are
 * generated by taking the square root of a Gamma variate.
 *
 * ### Example:
 * ```kotlin
...
public class NakagamiDistribution(
    ...
    init {
        if (!mu.isFinite() || mu < 0.5)
            throw InvalidParameterException("mu must be finite and >= 0.5, got $mu")
        if (!omega.isFinite() || omega <= 0.0)
            throw InvalidParameterException("omega must be finite and positive, got $omega")
    }
     ...
     *
     * Returns zero for negative values. At x = 0, the density depends on [mu]: it equals `sqrt(2 /
     * (pi * omega))` when mu is 0.5, and is zero when mu is greater than 0.5.
     *
     ...
     *
     * Uses the relationship Nakagami(mu, omega) CDF = GammaP(mu, mu * x squared / omega) where
     * GammaP is the regularized lower incomplete gamma function.
     *
    ...
    override val mean: Double
        get() = gammaRatio * sqrt(omega / mu)

    /** The variance of this distribution. */
    override val variance: Double
        get() = omega * (1.0 - gammaRatio * gammaRatio / mu)
    ...
    /**
     * The excess kurtosis of this distribution, computed from raw moments via the Gamma
     * relationship.
     */
    override val kurtosis: Double
        get() {
            ...
            val m3 = gammaRatio3 * (omega / mu).pow(1.5) // E[X^3]
            val m4 =
                (mu + 1.0) * omega * omega / mu // E[X^4] = Gamma(mu+2)/Gamma(mu) * (omega/mu)^2
            val v = variance
            return (m4 - 4.0 * m1 * m3 + 6.0 * m1 * m1 * m2 - 3.0 * m1 * m1 * m1 * m1) / (v * v) -
                3.0
        }
