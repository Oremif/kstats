 ...
 * failure modeling. It generalizes the exponential distribution: when [shape] equals 1 the Weibull
 * reduces to an exponential distribution with rate `1 / scale`. A shape less than 1 indicates a
 * decreasing failure rate (early-life failures), a shape of 1 indicates a constant failure rate
 * (random failures), and a shape greater than 1 indicates an increasing failure rate (wear-out
 * failures).
 *
 * The distribution is supported on the interval from zero to positive infinity. The [scale]
 * parameter stretches or compresses the distribution along the x-axis, while the [shape] parameter
 * controls how steeply the density rises and falls.
 *
 ...
 * ```
 *
 * @property shape the shape parameter (often denoted k), controlling the failure rate behavior.
 *   Must be positive.
 * @property scale the scale parameter (often denoted lambda), stretching the distribution along the
 *   x-axis. Must be positive. Defaults to 1.0.
 */
public class WeibullDistribution(
     ...
     *
     * The density is zero for negative values. At zero the behavior depends on [shape]: the density
     * is infinite when shape is less than 1, equals `1 / scale` when shape is exactly 1, and is
     * zero when shape exceeds 1.
     *
    ...
    override fun pdf(x: Double): Double {
        if (x < 0.0) return 0.0
        if (x == 0.0)
            return if (k == 1.0) 1.0 / lambda else if (k < 1.0) Double.POSITIVE_INFINITY else 0.0
        return exp(logPdf(x))
    }
     ...
     *
     * Gives the probability that a random variable drawn from this Weibull distribution is less
     * than or equal to [x]. Uses a closed-form expression.
     *
     ...
     *
     * Uses a closed-form expression based on the inverse of the CDF. No iterative root-finding is
     * needed.
     *
    ...
    override val skewness: Double
        get() {
            ...
            return (lambda * lambda * lambda * g3 - 3.0 * mu * sigma * sigma - mu * mu * mu) /
                (sigma * sigma * sigma)
        }
    ...
    override val kurtosis: Double
        get() {
            ...
            val mu2 = g2 - g1 * g1
            return (-6.0 * g1 * g1 * g1 * g1 + 12.0 * g1 * g1 * g2 - 3.0 * g2 * g2 - 4.0 * g1 * g3 +
                g4) / (mu2 * mu2) - 3.0
        }

    /**
     * Returns the Shannon entropy of this distribution in nats, using the Euler-Mascheroni
     * constant.
     */
