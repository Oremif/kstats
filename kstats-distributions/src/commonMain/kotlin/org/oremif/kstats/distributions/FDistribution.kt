 ...
 *
 * The F-distribution arises as the ratio of two independent chi-squared random variables, each
 * divided by its degrees of freedom. It is the workhorse distribution behind F-tests, which appear
 * in analysis of variance (ANOVA), regression significance testing, and comparing the variances of
 * two populations.
 *
 * The distribution is right-skewed and supported on the interval from zero to positive infinity. As
 * both degrees-of-freedom parameters grow, it converges toward a normal distribution. The mean
 * exists only when the denominator degrees of freedom exceeds 2, and the variance exists only when
 * it exceeds 4.
 *
 * Internally, the CDF and survival function use the regularized incomplete beta function, and the
 * quantile function uses Newton's method. Random sampling generates two independent chi-squared
 * variates and computes their ratio.
 *
 * ### Example:
 * ```kotlin
 ...
 * ```
 *
 * @property dfNumerator the degrees of freedom for the numerator (first) chi-squared variable. Must
 *   be positive.
 * @property dfDenominator the degrees of freedom for the denominator (second) chi-squared variable.
 *   Must be positive.
 */
public class FDistribution(
    ...
    init {
        if (!dfNumerator.isFinite() || dfNumerator <= 0.0)
            throw InvalidParameterException(
                "dfNumerator must be finite and positive, got $dfNumerator"
            )
        if (!dfDenominator.isFinite() || dfDenominator <= 0.0)
            throw InvalidParameterException(
                "dfDenominator must be finite and positive, got $dfDenominator"
            )
    }
     ...
     * values. Returns negative infinity for non-positive inputs.
     *
     ...
     *
     * Gives the probability that a random variable drawn from this F-distribution is less than or
     * equal to [x]. Computed using the regularized incomplete beta function for numerical
     * stability.
     *
     ...
     *
     * Computed directly using the regularized incomplete beta function with swapped parameters to
     ...
     *
     * Finds the value x such that `cdf(x) = p` using Newton's method with an initial guess based on
    ...
    override val entropy: Double =
        ln(d2 / d1) + lnBeta(d1 / 2.0, d2 / 2.0) + (1.0 - d1 / 2.0) * digamma(d1 / 2.0) -
            (1.0 + d2 / 2.0) * digamma(d2 / 2.0) + (d1 + d2) / 2.0 * digamma((d1 + d2) / 2.0)

    /**
     * Returns the mean, defined only when the denominator degrees of freedom exceeds 2. Returns
     * [Double.NaN] otherwise.
     */
    override val mean: Double
        get() = if (d2 > 2) d2 / (d2 - 2) else Double.NaN

    /**
     * Returns the variance, defined only when the denominator degrees of freedom exceeds 4. Returns
     * [Double.NaN] otherwise.
     */
    ...
    /**
     * Returns the skewness, defined only when the denominator degrees of freedom exceeds 6. Returns
     * [Double.NaN] otherwise.
     */
    ...
    /**
     * Returns the excess kurtosis, defined only when the denominator degrees of freedom exceeds 8.
