...
/**
 * Represents the Beta distribution, a continuous probability distribution defined on the interval
 * [0, 1].
 *
 * The Beta distribution is commonly used to model probabilities, proportions, and percentages
 * because its support is naturally bounded between zero and one. It is widely used as a conjugate
 * prior in Bayesian statistics -- for example, modeling the probability of success in a Bernoulli
 * trial before observing data. The two shape parameters [alpha] and [beta] control the shape of the
 * density: when both are equal, the distribution is symmetric around 0.5; when they differ, the
 * distribution skews toward one end.
 *
 * The CDF is computed via the regularized incomplete beta function, and quantiles are found using
 * Newton's method. Random samples are generated by taking the ratio of two independent
...
public class BetaDistribution(
    ...
    public companion object {
        /**
         * The standard Beta distribution with alpha = 1 and beta = 1, equivalent to the continuous
     ...
     *
     * Returns zero for values outside [0, 1]. At the boundaries, the density depends on the shape
     * parameters: it can be zero, finite, or infinite.
     *
     ...
     *
     * Returns the probability that a Beta-distributed random variable is less than or equal to [x],
     ...
     *
     * Uses a symmetry property of the regularized incomplete beta function for improved numerical
     ...
     *
     * Uses Newton's method seeded with the distribution mean as the initial guess. The result is
    ...
    override val kurtosis: Double
        get() { // excess
            val ab = alpha + beta
            return 6.0 *
                (alpha * alpha * alpha - alpha * alpha * (2.0 * beta - 1.0) +
     ...
     *
     * Generates two independent Gamma-distributed samples (one with shape [alpha], one with shape
     * [beta]) and returns their ratio. This is the standard method for Beta sampling.
     *
