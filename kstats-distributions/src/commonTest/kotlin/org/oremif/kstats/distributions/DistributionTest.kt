...
class DistributionTest {
    ...
    fun betaLogPdfBoundaryConsistency() {
        ...
        assertEquals(
            b1.pdf(0.0),
            kotlin.math.exp(b1.logPdf(0.0)),
            1e-12,
            "Beta(1,5): exp(logPdf(0)) ≈ pdf(0)",
        )
        // beta=1: pdf(1)=alpha, logPdf(1)=ln(alpha)
        val b2 = BetaDistribution(3.0, 1.0)
        assertEquals(
            b2.pdf(1.0),
            kotlin.math.exp(b2.logPdf(1.0)),
            1e-12,
            "Beta(3,1): exp(logPdf(1)) ≈ pdf(1)",
        )
    ...
    fun gammaLogPdfBoundaryConsistency() {
        ...
        assertEquals(
            g1.pdf(0.0),
            kotlin.math.exp(g1.logPdf(0.0)),
            1e-12,
            "Gamma(1,2): exp(logPdf(0)) ≈ pdf(0)",
        )
    ...
    fun chiSquaredLogPdfBoundaryConsistency() {
        ...
        assertEquals(
            c1.pdf(0.0),
            kotlin.math.exp(c1.logPdf(0.0)),
            1e-12,
            "ChiSq(2): exp(logPdf(0)) ≈ pdf(0)",
        )
        // df<2: both +Inf
    ...
    fun fDistributionLogPdfBoundaryConsistency() {
        ...
        assertEquals(
            f1.pdf(0.0),
            kotlin.math.exp(f1.logPdf(0.0)),
            1e-12,
            "F(2,10): exp(logPdf(0)) ≈ pdf(0)",
        )
        // d1>2: both 0 / -Inf
    ...
    fun nakagamiLogPdfBoundaryConsistency() {
        ...
        assertEquals(
            n1.pdf(0.0),
            kotlin.math.exp(n1.logPdf(0.0)),
            1e-12,
            "Nakagami(0.5,1): exp(logPdf(0)) ≈ pdf(0)",
        )
        // mu>0.5: both 0 / -Inf
    ...
    fun allDistributionsRejectNaNAndInfinityParameters() {
        val invalidFactories: List<Pair<String, () -> Unit>> =
            listOf(
                // Normal
                ...
                "Triangular(+Inf, 1, 0.5)" to
                    {
                        TriangularDistribution(Double.POSITIVE_INFINITY, 1.0, 0.5)
                    },
                // Geometric
                ...
                "BetaBinomial(10, +Inf, 1)" to
                    {
                        BetaBinomialDistribution(10, Double.POSITIVE_INFINITY, 1.0)
                    },
                "BetaBinomial(10, 1, +Inf)" to
                    {
                        BetaBinomialDistribution(10, 1.0, Double.POSITIVE_INFINITY)
                    },
            )
